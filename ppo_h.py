...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.conv4_actor = nn.Linear(self.a_dim, FEATURE_NUM)
        self.fc3_actor = nn.Linear(1, FEATURE_NUM)
        self.fc4_actor = nn.Linear(2, FEATURE_NUM)
        self.fc5_actor = nn.Linear(FEATURE_NUM * self.s_dim[0], FEATURE_NUM)
    ...
    def forward(self, inputs):
        ...
        split_5 = F.relu(self.conv4_actor(inputs[:, 5:6, :self.a_dim]).view(-1, FEATURE_NUM))
        split_6 = F.relu(self.fc3_actor(inputs[:, 6:7, -1]))
        split_7 = F.relu(self.fc4_actor(inputs[:, 7:8, :2])).view(-1, FEATURE_NUM)
        merge_net = torch.cat([split_0, split_1, split_2, split_3, split_4, split_5, split_6, split_7], 1)

        pi_net = F.relu(self.fc5_actor(merge_net))
...
class Critic(nn.Module):
    ...
    def forward(self, inputs):
        ...
        value_net = F.relu(self.fc5_actor(merge_net))
